👋 Hi , I'm Mrunali Patil

💻 Data Analyst | Python Developer | Power BI Enthusiast

<img align="right" alt="coding" width="350" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif">

🌱 Passionate about Data Analytics & AI
📊 Skilled in Python, SQL, Power BI & Tableau
🚀 Building real-world analytics and forecasting projects
🎯 Focused on transforming raw data into meaningful insights

---

 🌐 Connect With Me

* 💼 LinkedIn: https://www.linkedin.com/in/patil-mrunali
* 📧 Email: [patilmrunali775@gmail.com](mailto:patilmrunali775@gmail.com)

---

 🛠️ Languages and Tools

<p align="left">
<img src="https://skillicons.dev/icons?i=python,mysql,java,git,aws" />
</p>

 📊 Analytics Tools

* Power BI
* Tableau
* Excel
* Pandas
* NumPy
* Matplotlib
* Streamlit
---

🚀 Projects

### 📌 Sales Performance Dashboard

* Interactive Power BI dashboard for sales and customer analysis
* Visualized regional performance and customer trends

### 📌 Stock Price Forecasting System

* Forecasting using ARIMA, SARIMA, Prophet & LSTM
* Streamlit dashboard for comparison and analysis

### 📌 Healthcare Recommendation System

* NLP-based healthcare recommendation web app
* Chatbot symptom collection and analytics dashboard

### 📌 Customer Churn Analysis Dashboard

* Telecom churn analysis using Python, SQL & Power BI
* Customer retention and churn trend insights

---

## 🏆 Certifications

✔ Data Visualization – TATA & Forage
✔ AI-Powered Data Analytics – Tata iQ
✔ Deloitte Data Analytics Internship
✔ NASSCOM Certified in Data Analytics
✔ Generative AI Certification – EduBridge

---

⭐ “Turning data into actionable insights.”
